    ...
    */
    ...
    public function getZipsInRange($range_from, $range_to, $units=1)
    {
        ...
        $sql = "SELECT 3956 * 2 * ATAN2(SQRT(POW(SIN((RADIANS({$this->lat}) - "
              ...
              ."AND lat BETWEEN ROUND({$this->lat} - ($range_to / 69.172), 4) "
              ."AND ROUND({$this->lat} + ($range_to / 69.172), 4) "
              ."AND lon BETWEEN ROUND({$this->lon} - ABS($range_to / COS({$this->lat}) * 69.172)) "
              ."AND ROUND({$this->lon} + ABS($range_to / COS({$this->lat}) * 69.172)) "
